...
/**
 * Wie d1InsertMany, aber als Upsert: kollidiert eine Zeile mit conflictColumns,
 * wird sie überschrieben statt den Einfüge-Versuch scheitern zu lassen — und
 * ein vorhandener Grabstein (deleted_at) wird dabei aufgehoben.
 *
 * Gebraucht beim Zurücksetzen auf die Standard-Einrichtung: die alten Zeilen
 * werden vorher weich gelöscht (sonst bliebe der Reset für den Abgleich
 * unsichtbar, siehe app/api/reset/route.ts), tragen also schon wieder dieselbe
 * ID wie die Standardwerte, die gleich neu geschrieben werden — ein einfaches
 * INSERT würde an genau diesem Primärschlüssel scheitern.
 */
export async function d1UpsertMany(
  table: string,
  columns: string[],
